...
// Note: functionName and args must match the contract's "createProposal" function signature
const functionName = "createProposal";

type WriteResult = string | { data?: string; hash?: string; [key: string]: unknown };

interface UseCreateProposalsReturn {
    handleCreateProposal: (args: unknown[]) => Promise<string | null>;
    txHash?: string | undefined;
    isSigning: boolean;
    error: unknown | null;
}

export const useCreateProposals = (): UseCreateProposalsReturn => {
    ...
    async function handleCreateProposal(args: unknown[]): Promise<string | null> {
        try {
